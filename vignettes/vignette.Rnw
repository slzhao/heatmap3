...
\begin{itemize}
  ...
  \item More convenient coloring features: A more fancy color series is set as default color. You can balance the colors in color legend so that you can ensure the median color will represent the 0 value.
  ...
  \item Improvement in parameters: Pearson correlation is set as default method; the agglomeration method for clustering now can be specified; the input values can be transformed into matrix automatically if it is a data.frame; None is set as default value for parameter scale.
\end{itemize}
